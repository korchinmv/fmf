...
//окошко Подать заявку
	...
	$('.head_switcher_lang').click(function (e) {
		e.preventDefault();
		$('.langs').toggleClass('h');
	});
...
//фиксированое меню
	...
	$(window).scroll(function () {
		if ($(this).scrollTop() > 130 && $menu.hasClass("default")) {
...
//табы на странице контактов
	//$('.officeSectionTh').prepend('<div class="officeSectionTop"><div id="bangkok-tab" class="officeCity"><span>Bangkok</span></div><div id="moscow-tab" class="officeCity def"><span>Moscow</span></div><div id="samara-tab" class="officeCity def"><span>Samara</span></div></div>');
	$('.officeSectionTh').prepend('<div class="officeSectionTop"><div id="bangkok-tab" class="officeCity"><span>Bangkok</span></div></div>');
...
function clientsDetail($link) {
	...
	if (urlPage.indexOf('/clients/') > -1) {
		if ($('div').hasClass('mobile')) {
			if (!$link){
				$('.page-title .clientItem.prev a').attr('href', $('.clientsPage').find('.clientList .clientItem.prev a').attr('href'));
				$('.page-title .clientItem.next a').attr('href', $('.clientsPage').find('.clientList .clientItem.next a').attr('href'));
			}
		}

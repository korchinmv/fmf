...
?>
<div class="clients-list1 clients-list">
...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<div class="clients-item1 clients-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>" title="<?=$text;?>">
		<?if($link):?><a href="<?=$link;?>"><?else:?><div><?endif;?><img border="0" src="<?=$file['src']?>" width="<?=$file['width']?>" height="<?=$file['height']?>" alt="<?=$arItem["PREVIEW_PICTURE"]["ALT"]?>"/><?if($link):?></a><?else:?></div><?endif;?>
	</div>
<?endforeach;?>
	<div class="clear"></div>
</div>

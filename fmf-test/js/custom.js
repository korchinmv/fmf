...
$(".select-item").click(function(e) {
	...
	parent.siblings(".select-item-selected").find("input").val($(this).find("input").val());
});
...
$(window).scroll(function() {
	if (location.pathname == '/fmf-test/seo.html') {
		...
		if ($('body')[0].scrollTop >= $('.our-works-block').offset().top) {
			st = ($('body')[0].scrollTop - $('.types-block').offset().top) / $(window).height() * 100;
			$(".title-first").css({
	...
	if (location.pathname == '/fmf-test/context-ad.html') {
		if ((($('body')[0].scrollTop + $('.steps-block').offset().top) >= $(window).height()) && (($('body')[0].scrollTop + $('.steps-item-4').outerHeight(true)) <= $('.timeline-4').offset().top)) {
			st = ($('body')[0].scrollTop - $('.steps-block').offset().top);
			$(".timeline-status").css({
				"height" : st + "px"
			});
		}
		...
		let height2 = $('.steps-item-1').outerHeight(true) + $('.steps-item-2').outerHeight(true)
		let height3 = $('.steps-item-1').outerHeight(true) + $('.steps-item-2').outerHeight(true) + $('.steps-item-3').outerHeight(true)

		if (st > 0) {//console.log('1');
			$('.timeline-1').addClass('hover');
		} else {
			$('.timeline-1').removeClass('hover');
		}
		if (st > height1) {//console.log('1');
			$('.timeline-2').addClass('hover');
		} else {
			$('.timeline-2').removeClass('hover');
		}
		if (st > height2) {//console.log('1');
			$('.timeline-3').addClass('hover');
		} else {
			$('.timeline-3').removeClass('hover');
		}
		if (st > height3) {//console.log('1');
			$('.timeline-4').addClass('hover');
		} else {
...
if ($('#pillars').length) {
	if ($(window).width() > 768) {
		...
		var rightOffset = $(window).width() - ( $('#pillars .wrap_container').offset().left +  $('#pillars .wrap_container').width());
		let col = 3;
		...
		gsap.to("#pillars .pillars-container", {
			x: () => -(document.querySelector("#pillars .pillars-container").scrollWidth - window.innerWidth + (rightOffset * 2)),
			ease: "none", 
			scrollTrigger: {
				trigger: "#pillars .wrap_container", 
				start: "top top",
				end: () => "+=" + document.querySelector("#pillars .pillars-container").scrollWidth,
				scrub: true, 
				pin: true,
				anticipatePin: 1
			},
...
$('form.feedback-form').submit(function(e) {
	...
	if (phone.length) {
		...
		if (iti) {
			formData.set('phone', '+' + iti.selectedCountryData.dialCode + ' ' + phone.val());
		}
	}
	...
	$.ajax({
		...
		success: function(res) {
			...
			$this.parents('.feedback-content').siblings('.feedback-success').show();
		}
	})
});
